...
- **Native Liquid Glass floating bars** — With the iOS 26.2 deployment target, `ABYGlassBarBackground` now renders Apple's Liquid Glass (`.glassEffect(.regular, in:)`) instead of a static `.ultraThinMaterial`. Every floating surface built on it — the bottom nav pill, chat composer, passage search, scripture browse — gains dynamic light refraction and specular highlights that react to content behind them, while keeping the hairline stroke and drop shadow for separation.
- **Iridescent active-tab jewel** — The selected tab pill layers a restrained holographic sheen (`IridescentBubble.swift`) over the neutral glass capsule. It reuses the Sacred Orb palette (teal / sky / periwinkle / sage) at low opacity and drifts slowly, so the active tab reads as a specular "jewel" rather than a saturated fill. The sheen slides between tabs with the existing `matchedGeometryEffect` and honors Reduce Motion.
